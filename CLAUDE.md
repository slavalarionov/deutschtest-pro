...
### Workflow для крупных задач (feature-ветка)

**Правило:** для **крупных задач** (редизайны, многокоммитные фичи, рефакторы) — работаем через feature-ветку с финальным merge в `main`, не пушим прямо в `main`. Для **мелких правок** (один коммит, опечатка, маленький фикс) — продолжаем напрямую в `main`.

**Что считается крупной задачей:** 3+ коммита в одну логическую группу, 5+ файлов суммарно, новый UI под ревью пользователя, или работа, где есть шанс «разобрать на полпути и пересобрать иначе» по результатам ревью.

**Workflow:**
1. `git checkout -b feature/<short-name>-<date>` — создать ветку
2. Делать коммиты на feature-ветке
3. После каждого логического шага — мерж в main fast-forward (`git checkout main && git merge --ff-only feature/... && git push origin main`) для деплоя на прод и посекционного пользовательского ревью
4. Возвращаемся на feature-ветку для следующих коммитов: `git checkout feature/...`
5. После закрытия задачи — feature-ветку оставить как audit trail, не удалять

**В отчётах о деплое:** всегда показывать `git rev-parse origin/main` или `git log main --oneline | head -3`, чтобы явно подтвердить «коммит на проде = HEAD main». `git branch --show-current` показывает рабочую ветку, **не** ту, откуда деплоит Timeweb — не использовать как единственный источник состояния.
